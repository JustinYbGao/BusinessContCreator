...
export async function POST(request: Request, routeContext: RouteContext) {
  try {
    ...
    if (sourceIds.length > 0) {
      const { data: sources, error: sourceError } = await supabase.from("product_sources").select("id").eq("workspace_id", context.workspaceId).eq("product_id", productId).in("id", sourceIds);
      if (sourceError || (sources?.length ?? 0) !== sourceIds.length) throw new Error("SOURCE_SCOPE_MISMATCH");
    } else {
      const { data: sources, error: sourceError } = await supabase.from("product_sources").select("id").eq("workspace_id", context.workspaceId).eq("product_id", productId).order("created_at");
    ...
    const { data: job, error: jobError } = await supabase.from("workflow_jobs").insert({ workspace_id: context.workspaceId, product_id: productId, kind: "sync_product", idempotency_key: idempotencyKey, payload: { sourceIds }, status: "queued" }).select("id,status").single();
    ...
    const { error: auditError } = await supabase.rpc("append_audit_event", {
      ...
      p_event: { product_id: productId, actor_type: "user", actor_id: context.actorId, action: "workflow_job.enqueued", entity_type: "workflow_job", entity_id: job.id, request_id: requestId, payload: { kind: "sync_product", idempotencyKey, sourceIds } },
    });
    if (auditError) {
      await supabase.from("workflow_jobs").delete()
        .eq("workspace_id", context.workspaceId).eq("product_id", productId).eq("id", job.id).eq("status", "queued");
